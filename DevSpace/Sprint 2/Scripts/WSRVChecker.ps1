...
function MakeChoice{
    do
    {
        ...
        $Private:ScriptChoice2 = "C:\Temp\Scripts\RegistryKey\CheckRegistryKey.ps1"
        $Private:ScriptChoice3 = "C:\Temp\Scripts\RegistryKey\DefaultCode_1.ps1"
        ...
        switch ($Private:Selection)
        {
            "1" {
                ...
                Start-Process -FilePath "cmd.exe" -ArgumentList "/c powershell.exe -File $Private:ScriptChoice2 -ExecutionPolicy Bypass -FactoryAbbreviation $Private:FactoryAbbreviation -OutputFolder $Private:OutputFolder"
            } "3" {
                Write-Host "To start this script you need to set parameters" -ForegroundColor Yellow
                Write-Host "  "
                $Private:FactoryAbbreviation = Read-Host "Please enter the abbreviation of your factory ( CHORN | CHROM | CHAVE )"
                ...
                Start-Process -FilePath "cmd.exe" -ArgumentList "/c powershell.exe -File $Private:ScriptChoice3 -ExecutionPolicy Bypass -FactoryAbbreviation $Private:FactoryAbbreviation -OutputFolder $Private:OutputFolder"
            } "Q" {
                return
            }
        }
